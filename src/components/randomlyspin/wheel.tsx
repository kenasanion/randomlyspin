...
const BasicWheel: React.FC<BasicWheelProps> = ({
  entries,
  onSuccess: onWinner,
  onChange,
}) => {
  ...
  const startSpin = () => {
    ...
    const animate = (time: number) => {
      ...
      if (elapsed < spinDuration) {
        ...
        if (newWinner) {
          onWinner(newWinner);
          // playAudio("/sound/its_time_mariah.mp3", 16, 30);
        }
      }
    };
  ...
  const handleMouseUp = () => {
    if (lastMoveRef.current.length > 1) {
      ...
      console.log(
        "Release force (angular velocity):",
        velocity.toFixed(2),
        "deg/sec"
      );
    }
  ...
  const determineWinner = (
    participants: Entry[],
    previousWinner: Entry | null | undefined,
    ...
    if (participants.length > 0) {
      ...
      if (
        previousWinner &&
        previousWinner.name !== participants[winningSector].name
      ) {
  ...
  return (
    <div className="flex flex-col overflow-hidden items-center select-none">
      <div
